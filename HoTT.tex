...
\begin{itemize}
	\item Strukturalizm to filozofia matematyki, która twierdzi teorie matematyczne opisują strukturę obiektów matematycznych.
...
\section{Zdania, zbiory, grupoidy}
...
\begin{itemize}
	...
	\item $n$-typ, to typ, w którym wszystkie ścieżki powyżej $n$-tego poziomu są trywialne. $n$-typy bywają też nazywane typami $n$-obciętymi (ang. $n$-truncated).
	\item Dualnym pojęciem jest pojęcie $n$-spójności (ang. $n$-connectedness).
	\item Typ $n$-spójny to taki, w którym wszystkie ścieżki poniżej $n$-tego poziomu są trywialne.
	\item My zajmiemy się tylko typami $n$-obciętymi.
\end{itemize}
\end{frame}
...
Typ jest ściągalny, jeżeli ma ``centrum'', które jest połączone ścieżką z każdym innym punktem tego typu. \\~\

Nazwa ściągalny (ang. contractible) bierze się stąd, że intuicyjnie typ jest ściągalny, gdy możemy go ściągnąć do punktu poprzez ``ściąganie'' ścieżek.

\end{frame}

\begin{frame}{Ściągalność 2 - właściwości}
\begin{itemize}
	\item $\isContr(A) \to A \simeq \mathbf{1}$
	\item Oczywiście $\mathbf{1}$ jest ściągalny.
	\item Jeżeli $A$ i $B$ są ściągalne, to $A \times B$ oraz $A \to B$ też są, ale $A + B$ nie jest.
	\item Niech $A : \U$ i $B : A \to \U$, Jeżeli dla $x : A$ typ $B(x)$ jest ściągalny, to typ $\Pi x : A. B(x)$ też jest ściągalny (stwierdzenie to jest równoważne aksjomatowi ekstensjonalności).
	...
	\item Jeżeli $A$ jest ściągalny, to $\isContr(A)$ też jest ściągalny.
\end{itemize}
\end{frame}
...
\begin{itemize}
	...
	\item Jeżeli $f$ ma odwrotność, to ściągalny jest typ $\displaystyle \left(\sum_{g : B \to A} g \comp f \sim \id_A\right) \times \left(\sum_{h : B \to A} f \comp h \sim \id_B\right)$ mimo, że zupełnie podobny typ $\qinv(f)$ nie jest ściągalny.
\end{itemize}
\end{frame}
...
$A$ jest $-2$-typem, gdy $\isContr(A)$ \\
$A$ jest $(n + 1)$-typem, gdy dla $x, y : A$ typ $x = y$ jest $n$-typem.
\end{definition}

Idea jest prosta. Najtrywialniejsze typy to typy ściągalne, zaś typ na poziomie $n + 1$ może mieć dowolnie dużó punktów, ale wszystkie ścieżki między nimi są trywialne.
\end{frame}

\begin{frame}{Zdania 1 - definicja i intuicja}

$-1$-typy są zwane zdaniami. Odwijając definicję, dostajemy:

\begin{definition}[Zdanie]
$\isProp(A) \defn \prod_{x, y : A} x = y$
\end{definition}

Parafrazując, zdania to typy, które mogą mieć co najwyżej jeden element. Liczby się zatem istnienie elementu, a nie jego specyfika. Sytuacja ta przypomina klasyczne rozumienie logiki, gdzie liczy się przeprowadzenie dowodu, a nie jego postać.

\end{frame}

\begin{frame}{Zdania 2 - przykłady}
\begin{itemize}
	\item Oczywiście $\mathbf{0}$ i $\mathbf{1}$ są zdaniami. Odpowiadają one fałszowi i prawdzie.
	\item Każdy typ ściągalny jest zdaniem.
	\item Jeżeli $A$ jest zdaniem, to $\isProp(A)$ też jest zdaniem.
	\item Produkty, funkcje i funkcje zależne zachowują bycie zdaniem.
	\item Jednak suma i suma zależna nie zachowują bycia zdaniem. Świadczy to o tym, że są ``zbyt konstruktywne''.
\end{itemize}
\end{frame}

\begin{frame}{Zbiory 1 - definicja i intuicja}

W HoTT możemy zdefiniować także zbiory. Są to $0$-typy.

\begin{definition}[Zbiór]
$\isSet(A) \defn \prod_{x, y : A} \prod_{p, q : x = y} p = q$
\end{definition}

Intuicja jest prosta: zbiór to worek z kropkami. Wszystkie ścieżki między kropkami są trywialne.

\end{frame}

\begin{frame}{Zbiory - przykłady}
\begin{itemize}
	...
	\item Typ $\mathbb{N}$ jest zbiorem.
\end{itemize}
\end{frame}
